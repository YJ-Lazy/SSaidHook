...
public class MainActivity extends Activity {
    ...
    /**
     * LSPosed 是否已对本模块生效的检测点。
     * 默认 false；当模块在 LSPosed 中启用且作用域包含本应用时，
     * MainHook 会 hook 本方法恒返回 true。
     * 无论结果如何，桌面图标均可正常打开本应用，配置始终可编辑。
     */
    public boolean isModuleActive() {
        // 现代 API：模块应用不再被 self-hook，改为检测 libxposed Service 是否已绑定
        try {
            return App.isFrameworkBound();
        } catch (Throwable t) {
            return false;
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        statusBar.setPadding(dp(16), dp(10), dp(16), dp(10));
        boolean active = isModuleActive();
        if (active) {
            statusBar.setText("✓ LSPosed 已启用 — Hook 可对作用域内应用生效");
            statusBar.setTextColor(0xFF1B5E20);
            statusBar.setBackgroundColor(0xFFC8E6C9);
        } else {
            statusBar.setText("! LSPosed 未授权 / 未对本应用生效 — 仍可打开配置，但 Hook 不会生效（点此查看说明）");
            statusBar.setTextColor(0xFFE65100);
            statusBar.setBackgroundColor(0xFFFFE0B2);
            statusBar.setOnClickListener(v -> showLspUnauthorizedDialog());
        }
        ...
        // 未启用时弹出说明窗口（不拦截使用；异常不影响进入主界面）
        try {
            if (!isModuleActive()) {
                statusBar.post(() -> {
                    try {
                        showLspUnauthorizedDialog();
    ...
    private void showLspUnauthorizedDialog() {
        new AlertDialog.Builder(this)
                .setTitle("LSPosed 未授权 / 未启用")
                .setMessage(
                        "本应用可随时从桌面图标打开，配置的保存不受影响。\n\n"
                        + "当前检测：模块未在 LSPosed 中对本应用生效。\n\n"
                        + "若要让 SSAID Hook 生效，请：\n"
                        + "1. 安装并激活 LSPosed（推荐 Zygisk）\n"
                        + "2. 在 LSPosed 管理器中启用「SSaidHook」模块\n"
                        + "3. 作用域中勾选目标应用\n"
                        + "4. 保存配置后若已启用模块，将通过 libxposed Service 同步到框架\n"
                        + "5. 保存配置后，结束后台或强停目标应用再重新打开\n\n"
                        + "即使未启用 LSPosed，你仍可编辑、保存配置，待启用后即可生效。"
                )
                .setPositiveButton("我知道了", null)
                .setNeutralButton("打开 LSPosed", (d, w) -> {
                    String[] pkgs = {
                            "org.lsposed.manager",
                    ...
                    if (!opened) {
                        Toast.makeText(this, "未找到 LSPosed 管理器，请手动打开", Toast.LENGTH_LONG).show();
                    }
                })
                .show();
    }
